...
function init_log() {
  ...
  $("#search-form").submit(function(e) {
    ...
    $el.data("mode", "search");
    $("#log")
      .find(".controls .mode-text")
      .text("Search mode (Press enter for next, escape to exit)");
    ...
    $.get($log.data("search-log-url"), params, function(resp) {
      ...
      if (resp.position == -1) {
        ...
        var content_after = resp.content.slice(
          resp.buffer_pos + params["text"].length
        );
        ...
        // highlight matches
        var matched_text =
          '<span class="matching-text">' + params["text"] + "</span>";
        var found_text =
          '<span class="found-text">' + params["text"] + "</span>";
        content_before = replace_all(
          params["text"],
          matched_text,
          content_before
        );
        content_after = replace_all(
          params["text"],
          matched_text,
          content_after
        );
        ...
        $status.text(
          "Position " + resp.position + " of " + resp.filesize + "."
        );
      }
...
function init_connections_filter() {
  ...
  $content.on(
    "focus",
    "#connections-form select, #connections-form input",
    function() {
      skip_updates = true;
    }
  );
